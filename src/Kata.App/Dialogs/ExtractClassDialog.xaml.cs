...
public partial class ExtractClassDialog : Window
{
    private readonly ObservableCollection<MemberRow> _rows;

    public ExtractClassDialog(TypeNodeViewModel node)
    {
        InitializeComponent();

        SourceClassText.Text = node.Ref.FullyQualifiedName;
        NewClassNameBox.Text = node.Name + "Details";
        DelegateNameBox.Text = "Details";
    ...
    public string DelegatePropertyName { get; private set; } = string.Empty;
    ...
    {
        return member.Kind is MemberKind.Method or MemberKind.Property or MemberKind.Event or MemberKind.Field;
    }
    ...
    private void OnOkClick(object sender, RoutedEventArgs e)
    {
        ...
        var delegateName = DelegateNameBox.Text.Trim();
        if (string.IsNullOrWhiteSpace(name) || string.IsNullOrWhiteSpace(delegateName))
        {
            return;
        }
        ...
        NewClassName = name;
        DelegatePropertyName = delegateName;
